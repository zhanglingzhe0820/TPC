...
public class Client {
  ...
  private static void exportMeasurements(Properties props, int opcount, long runtime) throws IOException {
    TextMeasurementsExporter textMeasurementsExporter = null;
    ...
    try {
      ...
      try {
        exporter = (MeasurementsExporter) Class.forName(exporterStr).getConstructor(new Class[] { OutputStream.class }).newInstance(new Object[] { out });
      } catch (Exception e) {
        System.err.println("Could not find exporter " + exporterStr + ", will use default text reporter.");
        e.printStackTrace();
      }
      textMeasurementsExporter = new TextMeasurementsExporter(out);
      textMeasurementsExporter.write("OVERALL", "RunTime(ms)", runtime);
      double throughput = 1000.0D * opcount / runtime;
      textMeasurementsExporter.write("OVERALL", "Throughput(ops/sec)", throughput);
      ...
      for (Entry<String, Long[]> entry : gcs.entrySet()) {
        ...
        totalGCCount += ((Long[])entry.getValue())[0].longValue();
        totalGCTime += ((Long[])entry.getValue())[1].longValue();
      }
      textMeasurementsExporter.write("TOTAL_GCs", "Count", totalGCCount);
      textMeasurementsExporter.write("TOTAL_GC_TIME", "Time(ms)", totalGCTime);
      textMeasurementsExporter.write("TOTAL_GC_TIME_%", "Time(%)", totalGCTime / runtime * 100.0D);
      if (statusthread != null && statusthread.trackJVMStats()) {
        textMeasurementsExporter.write("MAX_MEM_USED", "MBs", statusthread.getMaxUsedMem());
        textMeasurementsExporter.write("MIN_MEM_USED", "MBs", statusthread.getMinUsedMem());
        textMeasurementsExporter.write("MAX_THREADS", "Count", statusthread.getMaxThreads());
        textMeasurementsExporter.write("MIN_THREADS", "Count", statusthread.getMinThreads());
        textMeasurementsExporter.write("MAX_SYS_LOAD_AVG", "Load", statusthread.getMaxLoadAvg());
        textMeasurementsExporter.write("MIN_SYS_LOAD_AVG", "Load", statusthread.getMinLoadAvg());
      }
      Measurements.getMeasurements().exportMeasurements((MeasurementsExporter)textMeasurementsExporter);
    } finally {
      if (textMeasurementsExporter != null)
        textMeasurementsExporter.close();
    }
  }
  ...
  public static void main(String[] args) {
    ...
    while (args[argindex].startsWith("-")) {
      if (args[argindex].compareTo("-threads") == 0) {
        ...
        props.setProperty("threadcount", String.valueOf(tcount));
        argindex++;
        ...
        props.setProperty("target", String.valueOf(ttarget));
        argindex++;
      } else if (args[argindex].compareTo("-c") == 0) {
        String client = args[argindex];
        props.setProperty("client", client);
        argindex++;
        ...
        for (Enumeration<?> enumeration = myfileprops.propertyNames(); enumeration.hasMoreElements(); ) {
    ...
    if (target > 0) {
      ...
      targetperthreadperms = targetperthread / 1000.0D;
    }
    Map<String, String> filteredProperties = new HashMap<>();
    for (String key : props.stringPropertyNames()) {
      if (key.startsWith("htrace."))
        filteredProperties.put(key.substring("htrace.".length()), props.getProperty(key));
    }
    HTraceConfiguration conf = HTraceConfiguration.fromMap(filteredProperties);
    ...
    Tracer tracer = (new Tracer.Builder("YCSB " + workload.getClass().getSimpleName())).conf(conf).build();
    try (TraceScope span = tracer.newScope("Client#workload_init")) {
      workload.init(props);
      warningthread.interrupt();
    } catch (WorkloadException workloadException) {
      workloadException.printStackTrace();
      workloadException.printStackTrace(System.out);
      System.exit(0);
    }
    CountDownLatch completeLatch = new CountDownLatch(threadcount);
    List<ClientThread> clients = new ArrayList<>(threadcount);
    boolean initFailed = false;
    int opcount = 0;
    try (TraceScope span = tracer.newScope("Client#init")) {
      if (dotransactions) {
        opcount = Integer.parseInt(props.getProperty("operationcount", "0"));
      } else if (props.containsKey("insertcount")) {
        opcount = Integer.parseInt(props.getProperty("insertcount", "0"));
      } else {
        opcount = Integer.parseInt(props.getProperty("recordcount", "0"));
      }
      double updatedcount = opcount + opcount * 0.04D;
    ...
    try (TraceScope span = tracer.newScope("Client#workload")) {
      Map<Thread, ClientThread> threads = new HashMap<>(threadcount);
    ...
    try (TraceScope span = tracer.newScope("Client#cleanup")) {
    ...
    try (TraceScope span = tracer.newScope("Client#export_measurements")) {
      exportMeasurements(props, opsDone, en - st);
    } catch (IOException iOException) {
      System.err.println("Could not export measurements, error: " + iOException.getMessage());
