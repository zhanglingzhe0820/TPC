  ...
  public void init() throws DBException {
    ...
    if (getProperties().containsKey("iotdbinfo")) {
      ...
      if (serverInfo.length != 3) {
        ...
        if (debug) {
          System.out
              .println(String.format("Parse IoTDB Server info succeed: %s:%d:%d", db_host, db_port,cache_threshold));
        }
      }
    }
    try {
    ...
    try {
      synchronized (CONNECTION_LOCK) {
        SessionDataSet dataSet = session
            .executeRawDataQuery(Collections.singletonList(deviceID), startTime, endTime);
